...
{
	if((strlen(reg)>0)&&(strlen(reg)<=3))
		return 1;
	else
		return 0;
}
int checksem(char bought_at_price[])
{
	if(strlen(bought_at_price)<=9)
		return 1;
	else
		return 0;
}
int checkbran(char player_country[])
{
      int valid=1;
	if(strcmpi(player_country,"India")||strcmp(player_country,"Foreign"))
		valid=0;
		  return valid;
}
int checkName(char *player_name)
{
	int valid=1,i;
	for(i=0;i<strlen(player_name);i++)
	if(isAlpha(player_name[i])==0)
	{
	     valid=0;
	     break;
	 }
...
class player
	 {
	  public:
		 char player_id[25];
		 char player_name[30];
		 char bought_by_team[40];
		 char bought_at_price[10];
		 char player_country[20];
		 void Clear();
		 int Unpack(fstream&);
		 int Pack(fstream&);
		 void Input(int);
...
class iobuf
	 {
		 ...
		 virtual int Pack(player&,fstream&)=0;
		 virtual int Unpack(player&,fstream&)=0;
	 };
...
class delmfld : public varlen
	 {
		 public:
		 int Pack(player&,fstream&);
		 int Unpack(player&,fstream&);
		};
btree bt;
SSET s;
node *dummy,*dummy1;
...
void update(char *key)
{
 ...
 int f=0,found=0,g;
 char upd='n';
 fstream file("player.dat",ios::in);
 file.seekg(0,ios::beg);
 while(!file.fail())
  if(std.Unpack(file))
	if(strcmpi(std.player_id,key)==0)
   {
    found=1;
	 cout<<"\n\tRecord:";
    std.Display();
...
//search for given record with player_id as key
...
void append()
{
  player std;
  int flag=1, pos;
  fstream file("player.dat",ios::app);
  std.Input(0);
  file.seekp(0,ios::end);
  pos=file.tellp();
  flag=s.Insert(std.player_id,pos);
...
////////////////////////////////////////////////////////////////////////////
//pack the player into Buffer with delimter and write into file
int delmfld::Pack(player &std,fstream  &file)
{
  Clear();
  strcpy(Buffer,std.player_id);
  strcat(Buffer,"|");
  strcat(Buffer,std.player_name);
  strcat(Buffer,"|");
  strcat(Buffer,std.bought_by_team);
  strcat(Buffer,"|");
  strcat(Buffer,std.bought_at_price);
  strcat(Buffer,"|");
  strcat(Buffer,std.player_country);
  strcat(Buffer,'\0');
  BufSiz=strlen(Buffer);
  return(Write(file));
}
//read record from file into Buffer and unpack into player
int delmfld::Unpack(player &std,fstream &file)
{
  ...
  {
    int p=0,q;
    for(q=0;Buffer[q]!='|';q++) std.player_id[p++]=Buffer[q];
    std.player_id[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.player_name[p++]=Buffer[q];
    std.player_name[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.bought_by_team[p++]=Buffer[q];
    std.bought_by_team[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.bought_at_price[p++]=Buffer[q];
    std.bought_at_price[p]='\0'; p=0; q++;
    for(;q<BufSiz;q++) std.player_country[p++]=Buffer[q];
    std.player_country[p]='\0';
    return 1;
  }
  return 0;
}
////////////////////////////////////////////////////////////////////////////
//clears player object
void player::Clear()
{
  strcpy(player_id,"");
  strcpy(player_name,"");
  strcpy(bought_by_team,"");
  strcpy(bought_at_price,"");
  strcpy(player_country,"");
}
//read data
void player::Input(int flUpd)
{
   Clear();
   if(!flUpd)
   {
   ...
   cout<<"\tPlayer Id: ";
   cin>>player_id;
   if(checkreg(player_id)==0){cout<<"\n Invalid Player Id\n";goto get_player_id;}
   }

   get_player_name:
   cout<<"\n\t player_name     : "; gets(player_name);
	if(checkName(player_name)==0)
	{
	cout<<"\n INVALID player_name \n";
	goto get_player_name;
	}

   cout<<"\n\t Bought by Team  : "; cin>>bought_by_team;
   get_bought_at_price:
   cout<<"\n\t Bought at Amount(in 8-figure Rupees) : "; cin>>bought_at_price;
   if(checksem(bought_at_price)==0){cout<<"\n INVALID Amount\n";goto get_bought_at_price;}
   get_player_country:
   cout<<"\n\t Player from   : "; cin>>player_country;
   if(checkbran(player_country)==1){cout<<"\n INVALID \n";goto get_player_country;}
   }

//display 1 player record
void player::Display()
{
  cout << "\n\n\t Player-Id     : " << player_id
	   << "\n\n\t player_name      : " << player_name
	   << "\n\n\t Bought by Team   : " << bought_by_team
	   << "\n\n\t Bought Price:       : " << bought_at_price
	   << "\n\n\t Player from    : " << player_country;
}
//assign given player to this player
void player::Assign(player &std)
{
  strcpy(player_id,std.player_id);
  strcpy(player_name,std.player_name);
  strcpy(bought_by_team,std.bought_by_team);
  strcpy(bought_at_price,std.bought_at_price);
  strcpy(player_country,std.player_country);
}
//packs player into new buffer
int player::Pack(fstream &file)
{
  delmfld buffer;
  return (buffer.Pack(*this,file));
}
//unpacks buffer into player
int player::Unpack(fstream &file)
{
  delmfld buffer;
